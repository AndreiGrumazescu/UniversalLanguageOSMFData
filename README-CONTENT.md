...
```
UL-Content/
├── shared-models/                               # Shared cross-language OSMF models
│   └── *.schema.json
├── {language}/
│   ├── data/
│   │   └── {model}/
...
Language directories contain language-local schemas/documents and tooling.
Shared reusable schemas that apply across languages live in `shared-models/`.
